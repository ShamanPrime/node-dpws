...
Service.prototype.handleAction = function (body, cb) {
  ...
  if (action === 'http://schemas.xmlsoap.org/ws/2004/08/eventing/Renew') {
    ...
    if (subscription._expirationTimer) {
    ...
    if (newExpiresAtText) {
      newExpiresAt = moment().add(newExpiresAtText)
      ...
      subscription._expirationTimer = newExpirationTimer
    }
    ...
    if (newExpiresAtText) {
      xml = xml.e('wse:Expires', util.msTo8601Duration(newMsToExpiration))
    }
  ...
  if (action === 'http://schemas.xmlsoap.org/ws/2004/08/eventing/Subscribe') {
    ...
    if (expiresAtText) {
      ...
      msToExpiration = expiresAt.diff(moment())
    ...
    if (expiresAtText) {
      xml = xml.e('wse:Expires', util.msTo8601Duration(msToExpiration)).up()
    }
  ...
  operation.handler(input, function (err, output) {
    ...
    var xml = xmlbuilder
      ...
      .att('xmlns:wsa', 'http://www.w3.org/2005/08/addressing')
      .e('s12:Header')
        .e('wsa:Action', self._operationOutputAction(operation.id)).up()
        .e('wsa:RelatesTo', msgId).up()
        .e('wsa:To', 'http://www.w3.org/2005/08/addressing/anonymous').up()
      .up()
      .e('s12:Body')
    ...
    if (operation.output) {
      ...
      xml = xml
        ...
        .att('xmlns:n2', 'http://www.w3.org/2001/XMLSchema-instance')

      if (typeof outputType === 'string') {
        xml = xml
          .att('xmlns:n3', 'http://www.w3.org/2001/XMLSchema')
          .att('n2:type', 'n3:' + outputType).txt(output)
      }
      else {
        xml = xml.att('n2:type', 'n1:' + outputTypeName + 'Type')

        var n = 3
        for (var paramName in output) {
          xml = xml
            .e('n1:' + paramName, output[paramName])
            .att('xmlns:n' + n, 'http://www.w3.org/2001/XMLSchema')
            .att('n2:type', 'n' + n + ':' + outputType[paramName])
            .up()
